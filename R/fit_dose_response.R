...
#' @param hill_slope_limits Numeric length-2 vector, shared by all assay
#'   types. Accepted for interface consistency with \code{fit_drc_4pl()} and
#'   \code{batch_drc_analysis()} and validated like the other limits, but has
#'   no effect here because the 3PL model fixes the Hill slope at
#'   \eqn{\pm 1}. Default \code{c(-5, 5)}.
#'
...
fit_drc_3pl <- function(data, output_file = NULL, normalize = FALSE, verbose = TRUE,
  ...
  # Note: hill_slope_limits is validated for interface consistency with
  # fit_drc_4pl()/batch_drc_analysis() but is otherwise unused here, because
  # the 3PL model fixes the Hill slope at +/-1.
  ...
  check_biological_plausibility <- function(params, df_clean) {
    ...
    if (length(corrections) > 0) {
      ...
      return(list(
        ...
        edge_response_detected = edge_response,
        fit_label = fit_label,
        # A no-response / out-of-window conclusion must also drive
        # curve_type <- "flat" in the caller, so the batch N/D marking agrees
        # with the quality label (otherwise a diverged fit on flat/drifting
        # data keeps a numeric IC50 next to an N/D label).
  ...
  analyze_single_pair <- function(pair_data, comp_name) {
    ...
    # no-response label regardless of how clean the fit looks.  When the
    # plausibility check supplied a more specific N/D reason (out-of-window
    # response), lead with that instead.
    nd_labels <- c("No dose response (inactive compound)",
                   "IC50 below tested range (N/D)",
                   "IC50 above tested range (N/D)")
    if (identical(curve_type, "flat")) {
      lead_label <- if (!is.null(check$fit_label) && check$fit_label %in% nd_labels) {
        check$fit_label
      } else {
        "No dose response (inactive compound)"
      }
      q_flags <- c(q_flags, lead_label)
    }
