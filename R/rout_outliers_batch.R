...
rout_outliers_batch <- function(batch_results,
  ...
  # Read the separator stamped by batch_ratio_analysis() / batch_viability_analysis()
  .sep <- attr(batch_results, "label_sep") %||% ":"
  ...
  .remove_na_columns <- function(tbl) {
    ...
    is_na_col <- vapply(value_cols, function(j) {
      ...
      cmpd_part <- strsplit(nm, .sep, fixed = TRUE)[[1L]][1L]
  ...
  # Preserve label_sep attribute so downstream functions can read it
  if (!is.null(attr(batch_results, "label_sep")))
    attr(output, "label_sep") <- attr(batch_results, "label_sep")
