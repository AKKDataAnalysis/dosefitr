#' Process viability data from a plate with row-based replicates (v3 layout)
#'
...
#'                       \code{selected_columns}.
#' @param split_replicates Accepted for API parity with v1. v3 never splits a
...
#' @param apply_control_means Logical. If \code{TRUE} (default), replace the
#'                       \code{control_0perc} and \code{control_100perc}
#'                       control cells with aggregated means (see
#'                       \code{control_mean_scope}). If \code{FALSE}, controls
#'                       are left as raw per-well values and
#'                       \code{control_mean_scope} is ignored.
#' @param control_mean_scope Character; one of \code{"construct"} (default),
#'                       \code{"row"}, or \code{"global"}. Only used when
#'                       \code{apply_control_means = TRUE}; it selects how the
#'                       0\% and 100\% control columns are aggregated (the same
#'                       scope is applied \emph{jointly} to both):
#'                       \describe{
#'                         \item{\code{"construct"}}{For each Target (grouped
#'                           by \code{info_table} Plate_Row -> Target), replace
#'                           that Target's rows' 0\% cells with the mean of those
#'                           0\% cells, and likewise for the 100\% cells. This is
#'                           the legacy per-construct behaviour and the default.}
#'                         \item{\code{"global"}}{Replace \emph{all} rows' 0\%
#'                           cells with the single plate-wide mean of the 0\%
#'                           column, and all rows' 100\% cells with the plate-wide
#'                           mean of the 100\% column.}
#'                         \item{\code{"row"}}{Identity: each plate row keeps
#'                           its own raw 0\% and 100\% control values (no
#'                           cross-row averaging). Because v3 holds a single
#'                           control well per row, there is nothing to average
#'                           \emph{within} a row, so \code{"row"} performs no
#'                           replacement. (This differs from \code{"row"} in the
#'                           NanoBRET-style \code{\link{process_viability_data_v2}},
#'                           which averages across multiple 100\% columns within
#'                           each row.)}
#'                       }
...
process_viability_data_v3 <- function(data,
  ...
  if (is.null(info_table))
    stop("v3 requires an info_table.")
  ...
  if (auto_detect) {
    ...
    if (is.null(pos))
      stop("v3 expects a 96-well (8x12) or 384-well (16x24) plate; no such block was detected in the input.")
  ...
  if (!(n_rows == 8L && n_cols == 12L) && !(n_rows == 16L && n_cols == 24L))
    stop(sprintf("v3 expects a 96-well (8x12) or 384-well (16x24) plate; detected %dx%d.",
                 n_rows, n_cols))
  ...
  if (is.null(control_0perc) || is.null(control_100perc))
    stop("v3 requires both control_0perc and control_100perc.")
  ...
  # -- 8. Apply control means (scope: construct / row / global) --------------
  # apply_control_means is the outer on/off gate. When TRUE, control_mean_scope
  # selects how the 0% and 100% control columns are aggregated (jointly to both):
  #   * "construct": per-Target means across that Target's rows (legacy default).
  #   * "global":    one plate-wide mean per control column, assigned to all rows.
  #   * "row":       identity -- each row keeps its own control value (no
  #                    averaging; v3 holds one control well per plate row).
  via_modified <- via_selected
  if (isTRUE(apply_control_means) && control_mean_scope != "row") {
    ...
    if (control_mean_scope == "global") {
      ...
      g0 <- mean(via_modified[, ctrl0_name], na.rm = TRUE)
      g1 <- mean(via_modified[, ctrl1_name], na.rm = TRUE)
      via_modified[, ctrl0_name] <- g0
      via_modified[, ctrl1_name] <- g1
      if (verbose)
        message(sprintf("Applying GLOBAL control means (0%%=%.3g, 100%%=%.3g).", g0, g1))

    } else {
      # ---- Per-Target means (legacy default) ---------------------------
  ...
  } else if (isTRUE(apply_control_means) && control_mean_scope == "row" && verbose) {
  ...
  result <- list(
    ...
    selected_columns_info = list(
      ...
      behavior           = "v3 (well indices map to plate columns; replicates from rows)"
    ),
    version       = "v3",
    data_type     = "viability",
    auto_detect   = auto_detect,
    behavior_mode = "v3-row-replicates"
  )
