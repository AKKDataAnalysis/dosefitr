...
**Consistent legend widths (`legend_label_width` and `legend_extract`):**

When generating multiple plots, legend widths can vary because shorter
labels produce narrower legends. Two parameters in
`plot_multiple_compounds()` address this:

- **`legend_label_width`** pads all legend labels to a fixed character
  width (shorter labels are right-padded with spaces; long labels are
  never truncated).

```{r legend-padding, eval=FALSE}
plot_multiple_compounds(drc_results, legend_label_width = 20)
```

- **`legend_extract`** returns the legend as a separate grid grob,
  guaranteeing all panels are identically sized. Combine with
  **patchwork**:

```{r legend-extract, eval=FALSE}
result <- plot_multiple_compounds(drc_results, legend_extract = TRUE)
# result$list contains $plot (no legend) and $legend (gtable grob)
